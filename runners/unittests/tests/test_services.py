    ...
    def test_get_users_eligible_for_giftcard(self):
        user_repository = UserMemoryRepository(
            _get_entities=lambda: (
                User(id=uuid4(), name='Jane Doe', points=1200, giftcards=[Giftcard(value=250, reason='welcome giftcard')]),  # Should be excluded
                User(id=uuid4(), name='John Doe', points=600),  # Should be excluded
                User(id=uuid4(), name='Jane Doe', points=1000),  # Should be included
                User(id=uuid4(), name='Jane Doe', points=999),  # should be excluded
            )
        )
        ...
        users_eligible_for_giftcard = list(user_service.get_users_eligible_for_giftcard())
